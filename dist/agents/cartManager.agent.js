...
async function handleFileOperations(action, product) {
    let cart = JSON.parse(fs.existsSync(CART_FILE) ? fs.readFileSync(CART_FILE, "utf8") : "[]");
...
async function handleRedisOperations(action, product) {
    const cartKey = "cart";
...
const cartManagerAgent = createReactAgent({
    ...
    stateModifier: new SystemMessage("You excel at managing add, remove, and list operations on the cart. Use the researcher's information to add new records, the supervisor's information to remove them, and show the cart."),
});
// Cart manager node
export const cartManagerNode = async (state, config) => {
    if (state.messages.some((msg) => msg.content.includes("Unable to retrieve"))) {
        return {
            messages: [
                new HumanMessage({
